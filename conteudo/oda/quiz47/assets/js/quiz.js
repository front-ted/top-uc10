$(function () {
    ...
    $("body").on("click", ".respostas button.bto", function () {
        ...
        let userOption = $(".top-slide").eq(idSlide).children(".respostas").children("button.bto").index(this);
        // let userOption = $(this).index();
        console.log(this, userOption, idSlide,respostasPai);
        ...
        if (perguntas[idSlide] && perguntas[idSlide].respostas[userOption]) {
            ...
            feedback((pergunta_atual-1), validacao, function () {
                pontuar(validacao);
            });
        } else {
...
let templateQuiz = function (i) {
    ...
    for (a = 0; a < nro_respostas; a++) {
        ...
        respostas.append('<button data-resp="' + a + '" type="button" data-label="' + resposta.ordem + '" class="bto">' + imagemHtml + resposta.texto + '</button>' + '<p class="legenda">');
    }
...
var verificarContainers = function (el) {
    ...
    console.log($(".top-slide").eq(pergunta_atual-1).find(".container-alvo").is(":empty"))
    ...
    if (coutEl >= $($(".top-slide")[pergunta_atual - 1]).find(".container-alvo").length) { // Verifica o contador e dispara a validação
        ...
        $($(".top-slide")[pergunta_atual - 1]).find(".container-alvo").addClass("respondido");
    } else {
        $("#navegacao").hide();
        $($(".top-slide")[pergunta_atual - 1]).find(".container-alvo").removeClass("respondido");
    }
}
